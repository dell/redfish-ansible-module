...
class RedfishUtils(object):
    ...
    def get_manager_attributes(self, uri):
        result = {}
        response = self.get_request(self.root_uri + self.manager_uri + uri)
        if response['ret'] is False:
            return response
        result['ret'] = True
        data = response['data']
    ...
    def set_manager_attributes(self, uri, attr):
        result = {}
        ...
        response = self.patch_request(self.root_uri + self.manager_uri + uri, payload, HEADERS)
        if response['ret'] is False:
            return response
        return {'ret': True}
